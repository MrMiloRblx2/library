...
local AstroLib = {
  Themes = {
    ...
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(0, 128, 255)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 20, 60))
   }),
    ["Color Hub 2"] = Color3.fromRGB(10, 15, 40),
    ["Color Stroke"] = Color3.fromRGB(0, 200, 255),
    ["Color Theme"] = Color3.fromRGB(0, 170, 255),
    ["Color Text"] = Color3.fromRGB(230, 250, 255),
    ["Color Dark Text"] = Color3.fromRGB(120, 170, 200)
    }
  ...
  Flags = {},
  Tabs = {},
  Icons = loadstring(game:HttpGet("https://raw.githubusercontent.com/MrMiloRblx2/library/refs/heads/main/Icons.lua"))()
}

local ViewportSize = workspace.CurrentCamera.ViewportSize
local UIScale = ViewportSize.Y / 450

local Settings = AstroLib.Settings
local Flags = AstroLib.Flags

local SetProps, SetChildren, InsertTheme, Create do
  InsertTheme = function(Instance, Type)
    table.insert(AstroLib.Instances, {
      Instance = Instance,
      Type = Type
    })
    return Instance
...
local Connections, Connection = {}, AstroLib.Connection do
...
local ScreenGui = Create("ScreenGui", PlayerGui, {
  Name = AstroLibName or "AstroLibrary",
}, {
  Create("UIScale", {
    Scale = UIScale,
    Name = "Scale"
...
local function VerifyTheme(Theme)
  for name,_ in pairs(AstroLib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local Theme = AstroLib.Themes[AstroLib.Save.Theme]

local function AddEle(Name, Func)
  AstroLib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = AstroLib.Elements[Ele](Instance, props, ...)
  return Element
end
...
-- /////////// --
function AstroLib:GetIcon(IconName)
...
function AstroLib:SetTheme(NewTheme, saveTheme)
  if not VerifyTheme(NewTheme) then return end
  
  AstroLib.Save.Theme = NewTheme
  if saveTheme == true then SaveJson("AstroLibrary.json", AstroLib.Save) end
  Theme = AstroLib.Themes[NewTheme]

  Connection:FireConnection("ThemeChanged", NewTheme)
  table.foreach(AstroLib.Instances, function(_,Val)
...
function AstroLib:SetScreenTitle()

end

function AstroLib:SetScale(NewScale)
  NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
  UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function AstroLib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "Astro Library"
  local WMiniText = Configs[2] or Configs.SubTitle or "by MrMiloRBLX And REDZ"
  ...
  if shared.AstroLibrary.ConnectMouse then
    shared.AstroLibrary.ConnectMouse:Disconnect()
    shared.AstroLibrary.ConnectMouse = nil
    UserInputService.MouseBehavior = Enum.MouseBehavior.LockCenter
  end

  if releasemouse then

    shared.AstroLibrary.ConnectMouse = RunService.RenderStepped:Connect(function()
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, AstroLib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
  ...
  ConnectSave(ControlSize1, function()
    if not Minimized then
      ...
      SaveJson("AstroLibrary.json", AstroLib.Save)
    end
  end)
  ...
  function Window:MakeTab(paste, Configs)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(AstroLib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(AstroLib.Options, {type = "Section", Name = SectionName, func = Section})
